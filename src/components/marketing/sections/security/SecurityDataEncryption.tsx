...
import { dataEncryptionSection } from "@/content/security";

const icons = { Lock, Database };

export function SecurityDataEncryption() {
  return (
        ...
        <Reveal>
          <ProseWidth large>
            <h2 className={`${sectionHeadline} text-mkt-text-primary`}>
              {dataEncryptionSection.headline}
            </h2>
            <p className="mt-5 text-xl leading-relaxed text-mkt-text-secondary">
              {dataEncryptionSection.copy}
            </p>
          </ProseWidth>
        </Reveal>

        <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2">
          {dataEncryptionSection.points.map((point) => {
