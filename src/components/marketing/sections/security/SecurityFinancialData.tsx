...
import { financialDataSection } from "@/content/security";

const icons = { ShieldCheck, CreditCard, Store };

export function SecurityFinancialData() {
  return (
        ...
        <Reveal>
          <ProseWidth large>
            <h2 className={`${sectionHeadline} text-mkt-text-primary`}>
              {financialDataSection.headline}
            </h2>
            <p className="mt-5 text-xl leading-relaxed text-mkt-text-secondary">
              {financialDataSection.copy}
            </p>
          </ProseWidth>
        </Reveal>

        <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-3">
          {financialDataSection.points.map((point) => {
