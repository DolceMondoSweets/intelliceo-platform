...
create table finance_data (
    ...
    -- V1 Budget vs. Actual: manually-entered monthly targets, nullable
    -- until a business sets a budget. Compared against revenue_mtd /
    -- monthly_cogs / monthly_labor_cost above — see calculateBudgetLine()
    -- in financial-formulas.ts.
    budgeted_revenue numeric,
    budgeted_cogs numeric,
    budgeted_labor numeric,
    updated_at timestamptz default now()
);

-- ── Goals (structured, trackable version of a Priority) ─────────────────
...
-- ── Per-business POS credentials (Square or Clover) ──────────────────────
-- One integration per business, self-service API keys generated by the
-- owner in their own POS platform's developer/merchant dashboard — not an
-- OAuth "Connect" flow. pos_type picks which fields apply: Square uses
...
create policy "Tenant isolation: goals"
    on goals for all
    using (business_id = (select business_id from profiles where id = auth.uid()));
...
create policy "Tenant isolation: pos_credentials"
